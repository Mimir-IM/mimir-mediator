...
impl Db {
    pub async fn open(path: &str) -> Result<Arc<Self>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        this.init_global_schema().await?;
        Ok(this)
    }

    async fn init_global_schema(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        self.conn
            .execute(
    ...
    pub async fn create_chat_tables(&self, id: i64) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let conn = &self.conn;
